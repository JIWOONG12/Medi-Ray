...
from evaluation import evaluate # evaluation.py의 evaluate
...
def train(args, epoch, loader, val_loader, model, device, optimizer, writer ,scheduler):
    ...
    now = time
    print(f"[ Train Start for Epoch {epoch} at {time.strftime('%Y-%m-%d %H:%M:%S')} ]")
    ...
    # 각 epoch이 끝난 후 현재 시간 출력
    print(f"[ Train End for Epoch {epoch} at {time.strftime('%Y-%m-%d %H:%M:%S')} ]")
...
def main(args):
    ...
    optimizer = torch.optim.AdamW(model.parameters(), # Sigmoid 파라미터 설정
                                  lr=args.lr, #learning_rate(학습률)
                                  weight_decay = 0.01,
                                  eps = 1e-07,
                                ) # weight_decay : L2 정규화를 통해 복잡성 줄이는데 사용 / 0 > 가중치 감소 적용 X
