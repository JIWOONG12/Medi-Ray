...
class ProgressMeter(object):
    def __init__(self, num_batches, meters, prefix=""):
        ...
        # 전체 배치 중 현재 배치가 몇 번째인지
        self.meters = meters
        # meters = 시간, 손실 값등?
        self.prefix = prefix # prefix = ""

    def display(self, batch):
        entries = [self.prefix + self.batch_fmtstr.format(batch)] # batch_fmrstr.format(batch) > 현재 배치정보
    ...
    def _get_batch_fmtstr(self, num_batches):
        ...
        return '[' + fmt + '/' + fmt.format(num_batches) + ']' # [ 현재 배치 / 총 배치 수 ]
